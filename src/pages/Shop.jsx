...
import { Search, ShoppingBag, ChevronRight, Filter, Eye } from 'lucide-react';
...
const Shop = () => {
  ...
  const filteredWatches = watchesData.filter(watch => {
    ...
    return brandMatch && styleMatch && priceMatch;
  });
            ...
            <div className="space-y-10">
              <div>
                ...
                <div className="space-y-4">
                  {['Rolex', 'Patek Philippe', 'Audemars Piguet', 'Cartier', 'Vacheron Constantin'].map(brand => (
                    <label key={brand} className="flex items-center gap-3 cursor-pointer group">
                      <div className="relative flex items-center justify-center">
                        <input 
                          type="checkbox" 
                          className="w-5 h-5 rounded border-primary/20 text-primary focus:ring-primary bg-transparent transition-all"
      ...
      <div className="flex-1">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          <div>
            ...
            <h1 className="text-5xl md:text-6xl font-black tracking-tighter dark:text-white uppercase leading-none">Curated <span className="text-primary italic font-serif lowercase font-light">Inventory</span></h1>
            ...
            >
              <div className="relative aspect-[3/4] overflow-hidden">
                ...
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-all duration-500 bg-background-dark/40 backdrop-blur-[2px] flex items-center justify-center gap-4">
                  <button className="p-4 rounded-full bg-primary text-background-dark shadow-2xl hover:scale-110 transition-transform">
                ...
                {watch.isNew && (
                  <span className="absolute top-6 left-6 bg-primary text-background-dark text-[9px] font-black uppercase tracking-[0.2em] px-3 py-1 rounded-full shadow-xl">New Arrival</span>
                )}
              </div>
              <div className="absolute bottom-0 left-0 right-0 p-8 pt-0">
                <p className="text-primary dark:text-white text-[9px] font-black uppercase tracking-[0.3em] mb-2">{watch.brand}</p>
                <h2 className="text-2xl font-serif italic text-slate-900 dark:text-white mb-2 leading-tight drop-shadow-sm">{watch.name}</h2>
        ...
        <div className="mt-24 flex flex-col items-center gap-8">
          ...
          <button className="px-16 py-5 bg-primary text-background-dark hover:brightness-110 transition-all duration-300 font-black text-[10px] uppercase tracking-[0.3em] rounded-xl shadow-xl shadow-primary/10">
